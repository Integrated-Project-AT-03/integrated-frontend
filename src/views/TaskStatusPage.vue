...
<template>
  ...
  <div class="container mx-auto flex flex-col gap-3" :class="route.fullPath.split('/').length > 2 && 'blur-sm'">
    ...
    <div class="w-full flex items-center justify-between">
      ...
      <div class="flex justify-end gap-4">
        <button class="itbkk-button-add btn btn-secondary text-slate-300" @click="router.push({ name: 'AddStatus' })">
          Add Status
        </button>
      </div>
    </div>
      ...
      <tbody class="bg-slate-100 divide-y divide-gray-300">
        ...
        <tr class="itbkk-item itbkk-button-action hover:bg-slate-200" v-for="(status, index) in datas.getStatuses()"
          ...
          <td class="itbkk-status flex py-4 whitespace-nowrap">
            <div v-if="status.id !== 1" class="flex justify-center items-center gap-2">
              <button class="btn bg-edit border-0" @click="
                router.push({ name: 'EditStatus', params: { id: status.id } })
                ">
                Edit
              </button>
              <button @click="() => (selectedStatus = { ...status, index })" class="btn btn-error"
  ...
  <TransferStatus @message="handleMessage" @close="() => (showTranferStauts = false)" v-if="showTranferStauts"
    :selected-status="selectedStatus" />

  <DeleteStatusModal @conflict="() => (showTranferStauts = true)" @message="handleMessage" :status="selectedStatus" />
