...
<template>
    <dialog id="my_modal_1" class="w-[65rem] m-auto h-[47rem] bg-neutral rounded-2xl">
        ...
        <div class="flex justify-around m-4">
            ...
            <div class="flex flex-col gap-10">
                <div class="flex flex-col gap-2 text-slate-200">
                    <div>Assignees</div>
                    <textarea class="itbkk-assignees w-[20rem] h-[12rem] rounded-2xl border p-4 text-slate-200 bg-secondary border-base-100">{{ dataModal?.assignees }}</textarea>
                </div>
                <div class="flex flex-col gap-2 text-slate-200">
                    <div>Status</div>
                    <select  class="itbkk-status select w-full max-w-xs bg-base-100"> 
                        <option :selected="dataModal?.status === 'no_status'">No status</option>
                ...
                <div class="flex flex-col h-3/4 gap-3 text-slate-200">
                    <div class="flex gap-2">
                        TimeZone:
                        <div class="itbkk-timezone">{{ Intl.DateTimeFormat().resolvedOptions().timeZone }}</div>
                    </div>
                    <div class="flex gap-2">
                        Created On:
                        <div class="itbkk-created-on">{{ dataModal?.createdOn }}</div>
                    </div>
                    <div class="flex gap-2">
                        Updated On:
                        <div class="itbkk-updated-on">{{ dataModal?.updatedOn }}</div>
                    </div>
                </div>
            </div>
        </div>
